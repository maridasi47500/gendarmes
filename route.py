...
class Route():
    def __init__(self):
        ...
        self.mysession={"notice":None,"gender":None,"email":None,"name":None}
        self.dbUsers=User()
        self.dbSport=Sports()
    ...
    def new(self,search={}):
        return self.render_figure.render_figure("sport/new.html")
    def createnew(self,params={}):
        myparams=self.get_post_data()(params=("seconds","minutes","pompe","abdo",))
        self.user=self.dbSport.create(myparams)
        if self.user["sport_id"]:
          self.set_notice(self.user["notice"])
          self.set_json("{\"redirect\":\"/mysport/"+self.user["sport_id"]+"\"}")
        else:
          self.set_json("{\"redirect\":\"/sport\"}")
        return self.render_figure.render_json()
    ...
    def seenew(self,params={}):
        ...
        print(myparam)
        self.render_figure.set_param("sport",Sports().getbyid(myparam["id"]))
        return self.render_figure.render_figure("sport/show.html")
    ...
    def mysports(self,params={}):
        self.render_figure.set_param("mysports",Sports().getall())
        return self.render_figure.render_figure("sport/allsport.html")
    ...
    def save_user(self,params={}):
        ...
        myparam=self.get_post_data()(params=("minutes","seconds","abdo","pompe","businessaddress","gender","profile","metier", "otheremail", "password","zipcode", "email", "mypic","postaladdress","nomcomplet","password_confirmation"))
    ...
    def run(self,redirect=False,redirect_path=False,path=False,session=False,params={},url=False,post_data=False):
        ...
        if path and path.endswith("jpg"):
            ...
            ROUTES={
                    "^/allmysports$":self.mysports,
                    "^/mysport/([0-9]+)$":self.seenew,
